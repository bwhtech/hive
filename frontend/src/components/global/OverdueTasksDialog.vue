<template>
	...
	>
		<template #default>
			<p class="mb-3 text-p-base text-ink-gray-6">{{ message }}</p>
			<div class="max-h-[50vh] space-y-1 overflow-y-auto">
				<button
					v-for="task in tasks"
					:key="task.name"
					type="button"
					class="flex w-full items-start gap-3 rounded-3 p-3 text-left hover:bg-surface-gray-2"
					@click="openTask(task)"
				>
					<span
					...
					<div class="min-w-0 flex-1">
						<p class="truncate text-sm font-medium text-ink-gray-8">{{ task.title }}</p>
						<div class="mt-1 flex items-center gap-2">
							...
							<Badge
								...
								variant="subtle"
							/>
							<span class="whitespace-nowrap text-xs text-ink-red-6">
