...
Status: IN PROGRESS. W0, all of phase 1, W9, W10 and W11 landed (2026-08-28). W12 remains (separate PR off `develop`). Replaces `frontend/` (React 19 + shadcn, ~16.7k LOC) with a Vue 3 app built on `frappe-ui@1.0.0-beta.55`. Agent features are dropped. Backend stays as is except for the small items in §9.
...
| **W11 Remove React** | **Done.** `frontend-react/` deleted (121 tracked files). Root `package.json` already pointed at `frontend/`, so nothing to prune. `CLAUDE.md` rewritten: Vue 3 + frappe-ui, the `yarn dev` / `yarn build` / `yarn lint` / `yarn format` / `yarn typecheck` commands, the frappe-ui skill and design tokens, and `useList` / `useDoc` / `useCall` in place of the frappe-react-sdk section. The React-only `vercel-react-best-practices` note is gone. README gained a Development section. |
